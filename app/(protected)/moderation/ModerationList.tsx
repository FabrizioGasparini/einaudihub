...
export default function ModerationList({ reports }: { reports: ReportItem[] }) {
    ...
    const requestHide = (reportId: string, type: 'post' | 'comment', contentId: string) => {
        setConfirmState({ isOpen: true, reportId, type, contentId });
    };

    const performHide = async () => {
        if (!confirmState) return;
        const { reportId, type, contentId } = confirmState;
    ...
    return (
        <>
            <ConfirmModal
                ...
                onConfirm={performHide}
                title="Nascondi Contenuto"
                message="Sei sicuro di voler nascondere questo contenuto? Sarà rimosso dalla visualizzazione pubblica."
                isDestructive
            />
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex justify-end gap-3">
                                    ...
                                    {report.post && (
                                        <button 
                                            onClick={() => requestHide(report.id, 'post', report.post!.id)}
                                            disabled={loading === report.id}
                                    ...
                                    {report.comment && (
                                        <button 
                                            onClick={() => requestHide(report.id, 'comment', report.comment!.id)}
                                            disabled={loading === report.id}
                                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-600 text-white text-sm font-bold hover:bg-red-700 disabled:opacity-50"
                                        >
                                            <EyeOff size={16} />
